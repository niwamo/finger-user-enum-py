...
def test_users(userlist, found_users, rhost):
...
def main():
    ...
    for i in range(args.threadcount):
        ...
        thrd = threading.Thread(target=test_users, args=(test_subset, found_users, args.rhost,))
        threads.append(thrd)
        thrd.start()
    ...
    if args.out:
        ...
        print(f"\n{'-'*40}Script Finished. Found {len(found_users)} usernames.\n{'-'*40}")
        for user in found_users:
            print(user)
